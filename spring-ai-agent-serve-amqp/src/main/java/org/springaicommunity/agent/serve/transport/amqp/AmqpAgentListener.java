...
/**
 * AMQP consumer that routes inbound messages to agent sessions.
 *
 * <p>
 * Listens on two queues:
 * <ul>
 *   <li>Requests queue — receives {@link AgentRequest} messages, creates or retrieves
 *       the session, and submits the request for streaming processing.</li>
 *   <li>Answers queue — receives {@link QuestionAnswer} messages and resolves pending
 *       questions on the corresponding session.</li>
 * </ul>
 *
 * <p>
 * This is the AMQP equivalent of the SSE controller's {@code sendMessage()} and
 ...
 *
 */
public class AmqpAgentListener {

	private static final Logger logger = LoggerFactory.getLogger(AmqpAgentListener.class);

	private final AgentSessionManager sessionManager;

	private final AgentEventSender eventSender;

	public AmqpAgentListener(AgentSessionManager sessionManager, AgentEventSender eventSender) {
		this.sessionManager = sessionManager;
		this.eventSender = eventSender;
	}

	@RabbitListener(queues = "${spring.ai.agent.serve.amqp.request-queue:agent-requests}",
			containerFactory = "agentServeRabbitListenerContainerFactory")
	public void onRequest(AgentRequest request) {
		String sessionId = request.sessionId();
		logger.debug("Received request from AMQP for session [{}]", sessionId);
	...
	@RabbitListener(queues = "${spring.ai.agent.serve.amqp.answer-queue:agent-answers}",
			containerFactory = "agentServeRabbitListenerContainerFactory")
	public void onAnswer(QuestionAnswer answer) {
		String sessionId = answer.sessionId();
		logger.debug("Received answer from AMQP for session [{}], question [{}]",
